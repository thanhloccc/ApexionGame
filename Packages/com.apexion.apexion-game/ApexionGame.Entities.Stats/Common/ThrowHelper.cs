...
namespace ApexionGame.Entities.Stats
{
    ...
    internal static class ThrowHelper
    {
        ...
        [Conditional(UNITY_EDITOR), Conditional(DEBUG)]
        [Conditional(RUNTIME_CHECKS), Conditional(STATS_CHECKS)]
        internal static void ThrowIfStoreIsNotCreated([DoesNotReturnIf(false)] bool isCreated)
        {
            if (isCreated == false)
            {
                throw CreateException();
            ...
            [MethodImpl(MethodImplOptions.NoInlining)]
            static ArgumentException CreateException()
                => new("Stat store is not created or has already been disposed", "store");
        }

        [HideInCallstack, StackTraceHidden]
        [Conditional(UNITY_EDITOR), Conditional(DEBUG)]
        [Conditional(RUNTIME_CHECKS), Conditional(STATS_CHECKS)]
        internal static void ThrowIfOwnerIsDead([DoesNotReturnIf(false)] bool isAlive, StatOwnerHandle owner)
        {
            if (isAlive == false)
            {
                throw CreateException(owner);
            }

            [MethodImpl(MethodImplOptions.NoInlining)]
            static ArgumentException CreateException(StatOwnerHandle owner)
                => new($"Stat owner '{owner}' does not exist in this store, or has already been destroyed.", "owner");
        }
