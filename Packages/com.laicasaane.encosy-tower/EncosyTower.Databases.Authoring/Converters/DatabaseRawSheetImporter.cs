...
namespace EncosyTower.Databases.Authoring
{
    ...
    public abstract class DatabaseRawSheetImporter : ISheetImporter, ISheetFormatter
    {
        ...
        private static NamingMap BuildNamingMap(ISheet sheet, string sheetName, NameCasing nameCasing)
        {
            ...
            while (baseSheetType != null)
            {
                var nesedTypes = baseSheetType.GetNestedTypes();
                uniqueTypes.EnsureCapacity(uniqueTypes.Count + nesedTypes.Length);

                foreach (var nestedType in nesedTypes)
                {
                    uniqueTypes.Add(nestedType);
                }

                baseSheetType = baseSheetType.BaseType;
            }
