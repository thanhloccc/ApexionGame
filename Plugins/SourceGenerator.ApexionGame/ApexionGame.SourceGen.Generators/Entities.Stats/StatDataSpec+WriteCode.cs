namespace ApexionGame.SourceGen.Generators.Entities.Stats
{
    partial struct StatDataSpec
    {
        ...
        private const string PR_GENERATED_CODE = $"[SCDC.GeneratedCode(\"ApexionGame.SourceGen.Generators.Entities.Stats.StatDataGenerator\", \"{SourceGenVersion.VALUE}\")]";

        private const string PR_ISTAT_DATA = "AGES.IStatData";
        private const string PR_STAT_VARIANT = "AGES.StatVariant";
        private const string PR_STAT_VARIANT_TYPE = "AGES.StatVariantType";
        ...
        private readonly void WriteIStatData(ref Printer p)
        {
            ...
            WriteValueProperty(ref p, "BaseValue", singleValue ? "value" : "baseValue");
            WriteValueProperty(ref p, "CurrentValue", singleValue ? "value" : "currentValue");
        }

        private readonly void WriteValueProperty(ref Printer p, string propertyName, string fieldName)
        {
            p.PrintBeginLine("public ").Print(PR_STAT_VARIANT).Print(" ").PrintEndLine(propertyName);
            p.OpenScope();
            {
                p.PrintLine(PR_AGGRESSIVE_INLINING);
                ...
                p.OpenScope();
                {
                    ...
                    p.OpenScope();
                    {
                        ...
                        p.Print("value.").Print(valueTypeName).PrintEndLine(";");
                    }
                    p.CloseScope();
                    p.Print(PR_IF_DEBUG).PrintEndLine();
                    p.PrintLine("else");
                    p.OpenScope();
                    {
                        p.PrintBeginLine(PR_LOG_ERROR).Print("($\"The setter of '").Print(typeName).Print(".")
                            .Print(propertyName).Print("' ")
