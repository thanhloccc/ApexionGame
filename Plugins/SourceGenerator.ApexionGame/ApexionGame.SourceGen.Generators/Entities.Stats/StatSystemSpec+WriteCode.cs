...
namespace ApexionGame.SourceGen.Generators.Entities.Stats
{
    partial struct StatSystemSpec
    {
        ...
        private const string GENERATOR = "\"ApexionGame.SourceGen.Generators.Entities.Stats.StatSystemGenerator\"";
        ...
        // The four ECS storage primitives, replaced per doc §1.5.
        // IBufferElementData / IBaker / ComponentTypeSet / EntityManager / EntityCommandBuffer /
        // SystemState have no counterpart; the code that emitted them is removed instead.
        private const string OWNER = $"AGES.StatOwnerHandle";
        private const string STAT_STORE = $"AGES.StatStore<Stat, StatModifier, StatObserver>";
        private const string STAT_STORE_XML = $"AGES.StatStore{{TStat, TStatModifier, TStatObserver}}";
        private const string STAT_BUFFER = $"AGES.StatBuffer<Stat>";
        private const string LOOKUP_STAT = $"AGES.StatBufferLookup<Stat>";
        private const string LOOKUP_OBSERVER = $"AGES.StatBufferLookup<StatObserver>";
        private const string LOOKUP_MODIFIER = $"AGES.StatBufferLookup<StatModifier>";
        private const string LOOKUP_STAT_XML = $"AGES.StatBufferLookup{{TStat}}";
        private const string STAT_BUFFER_XML = $"AGES.StatBuffer{{TStat}}";
        private const string LOOKUP_OBSERVER_XML = $"AGES.StatBufferLookup{{TStatObserver}}";
        private const string LOOKUP_MODIFIER_XML = $"AGES.StatBufferLookup{{TStatModifier}}";
        ...
        private const string STAT_SINGLE = $"AGES.StatSingle";
        private const string MODIFIER_RANGE = $"AGES.ModifierRange";
        private const string OBSERVER_RANGE = $"AGES.ObserverRange";
        private const string ISTAT_DATA = $"AGES.IStatData";
        private const string ISTAT_VALUE_PAIR = $"AGES.IStatValuePair";
        private const string ISTAT_VALUE_PAIR_COMPOSER = $"AGES.IStatValuePairComposer<ValuePair>";
        private const string ISTAT = $"AGES.IStat<ValuePair>";
        private const string ISTAT_MODIFIER_STACK = $"AGES.IStatModifierStack<ValuePair, Stat>";
        private const string ISTAT_MODIFIER = $"AGES.IStatModifier<ValuePair, Stat, StatModifier.Stack>";
        private const string ISTAT_OBSERVER = $"AGES.IStatObserver";
        private const string STAT_VARIANT_TYPE = $"AGES.StatVariantType";
        private const string STAT_VARIANT = $"AGES.StatVariant";
        private const string STAT_OWNER_REMAP = $"AGES.StatOwnerRemap";
        private const string STAT_HANDLE = $"AGES.StatHandle";
        private const string STAT_HANDLE_T = $"AGES.StatHandle<TStatData>";
        private const string STAT_MODIFIER_HANDLE = $"AGES.StatModifierHandle";
        private const string STAT_READER = $"AGES.StatReader<ValuePair, Stat>";
        private const string STAT_VARIANT_TYPE_EXCEPTION = $"AGES.StatVariantTypeException";
        private const string STAT_CHANGE_EVENT = $"AGES.StatChangeEvent<ValuePair>";
        private const string MODIFIER_TRIGGER_EVENT = $"AGES.ModifierTriggerEvent<{TYPES_4}>";
        private const string STAT_MODIFIER_RECORD = $"AGES.StatModifierRecord<{TYPES_4}>";
        private const string STAT_DATA_PARAMS = $"AGES.StatDataParams<TStatData>";
        private const string STAT_VALUE_PARAMS = $"AGES.StatValueParams<ValuePair>";
        private const string STAT_DATA_PARAMS_XML = $"AGES.StatDataParams{{TStatData}}";
        private const string STAT_VALUE_PARAMS_XML = $"AGES.StatValueParams{{TValuePair}}";
        private const string STAT_READER_XML = $"AGES.StatReader{{TValuePair, TStat}}";
        private const string STAT_HANDLE_T_XML = $"AGES.StatHandle{{TStatData}}";
        private const string STAT_MODIFIER_RECORD_XML = $"AGES.StatModifierRecord{{{TYPES_4_T}}}";
        private const string STAT_CHANGE_EVENT_XML = $"AGES.StatChangeEvent{{TValuePair}}";
        private const string MODIFIER_TRIGGER_EVENT_XML = $"AGES.ModifierTriggerEvent{{{TYPES_4_T}}}";
        ...
        private const string STAT_API = $"AGES.StatAPI";
        private const string STAT_WORLD_DATA = $"AGES.StatWorldData<{TYPES_5}>";
        private const string STAT_ACCESSOR = $"AGES.StatAccessor<{TYPES_6}>";
        private const string STAT_ACCESSOR_READONLY = $"AGES.StatAccessor<{TYPES_6}>.ReadOnly";
        private const string STAT_BUILDER = $"AGES.StatBuilder<{TYPES_6}>";
        private const string DEFERRED_LIST_JOB = $"AGES.DeferredUpdateStatListJob<{TYPES_6}>";
        private const string DEFERRED_QUEUE_JOB = $"AGES.DeferredUpdateStatQueueJob<{TYPES_6}>";
        private const string DEFERRED_STREAM_JOB = $"AGES.DeferredUpdateStatStreamJob<{TYPES_6}>";
        private const string DEFERRED_UNSAFE_BLOCK_LIST_JOB = $"AGES.DeferredUpdateStatUnsafeBlockListJob<{TYPES_6}>";
        private const string STAT_WORLD_DATA_XML = $"AGES.StatWorldData{{{TYPES_5_T}}}";
        private const string STAT_ACCESSOR_XML = $"AGES.StatAccessor{{{TYPES_6_T}}}";
        private const string STAT_ACCESSOR_READONLY_XML = $"AGES.StatAccessor{{{TYPES_6_T}}}.ReadOnly";
        private const string STAT_BUILDER_XML = $"AGES.StatBuilder{{{TYPES_6_T}}}";
        ...
        private const string NATIVE_SET_OWNER = $"UC.NativeHashSet<{OWNER}>";
        ...
        private const string NATIVE_SET_OWNER_XML = $"UC.NativeHashSet{{{OWNER}}}";
        ...
        public readonly string WriteCode(References references)
        {
            ...
            p = p.IncreasedIndent();
            {
                WriteStatData(ref p, syntaxKeyword, statSystemName);
                WriteStat(ref p, userDataSize, userDataType, syntaxKeyword, statSystemName);
                WriteStatObserver(ref p, syntaxKeyword, statSystemName);
                WriteStatModifier(ref p, syntaxKeyword, statSystemName);
                WriteWrappers(ref p, syntaxKeyword, statSystemName);
                WriteJobs(ref p, references, syntaxKeyword, statSystemName);
                WriteValuePair(ref p, syntaxKeyword, statSystemName);
                WriteIsCompatibleMethod(ref p, typeName, singleTypes, pairTypes, syntaxKeyword, statSystemName);

                WriteStatImpl(ref p, syntaxKeyword, statSystemName);
                WriteAPIImpl(ref p, syntaxKeyword, statSystemName);
                WriteReaderImpl(ref p, syntaxKeyword, statSystemName);
                WriteAccessorImpl(ref p, syntaxKeyword, statSystemName);
                WriteAccessorReadOnlyImpl(ref p, syntaxKeyword, statSystemName);
                WriteBakerImpl(ref p, syntaxKeyword, statSystemName);
                WriteWorldDataImpl(ref p, syntaxKeyword, statSystemName);
                WriteValuePairImpl(ref p, typeName, maxDataSize, singleTypes, pairTypes, incompatTypes, syntaxKeyword, statSystemName);
                WriteStatDataStore(ref p, singleTypes, pairTypes, syntaxKeyword, statSystemName);
                WritePairTypes(ref p, pairTypes, syntaxKeyword, statSystemName);
                WriteHelpersMethods(ref p, typeName, syntaxKeyword, statSystemName);
            }
            p = p.DecreasedIndent();
        ...
        {
            var types = StatTypeTable.Types.AsSpan();
            var typeNames = StatTypeTable.TypeNames.AsSpan();
            var sizes = StatTypeTable.Sizes.AsSpan();
            var namespaces = StatTypeTable.Namespaces.AsSpan();
        ...
        private static void WriteStatData(
              ref Printer p
            , string syntaxKeyword
            , string statSystemName
        )
        {

            p.Print("#region    STAT<TSTATDATA>").PrintEndLine();
            p.Print("#endregion ===============").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Stat<TStatData>");
            p.OpenScope();
            {
        ...
        {
            p.Print("#region    STAT").PrintEndLine();
            p.Print("#endregion ====").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Stat");
            p.OpenScope();
            {
        ...
        private static void WriteStatImpl(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            p.Print("#region    IMPL - STAT").PrintEndLine();
            p.Print("#endregion ===========").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: Stat");
            p.OpenScope();
            {
            ...
            p.Print("#region    IMPL - STAT<TSTATDATA>").PrintEndLine();
            p.Print("#endregion ======================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: Stat<TStatData>");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
        ...
        private static void WriteStatObserver(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("StatObserver");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
                p.PrintBeginLine("public partial struct StatObserver ")
                    .Print(": ").Print(ISTAT_OBSERVER)
                    .PrintEndLine();
                p.OpenScope();
                {
                    p.PrintLine(SERIALIZED_FIELD);
        ...
        private static void WriteStatModifier(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("StatModifier");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
                p.PrintBeginLine("public partial struct StatModifier ")
                    .Print(": ").Print(ISTAT_MODIFIER)
                    .PrintEndLine();
                p.OpenScope();
                {
                    ...
                    // DEC-004: not present upstream. A StatHandle embeds StatOwnerHandle.Index/Version,
                    // so every handle a modifier stores must be rewritten after a save/load. The
                    // generator cannot know which fields hold handles, so it defers to a partial the
                    // user fills in — symmetric with AddObservedStatsToList.
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public void RemapObservedStats(in ").Print(STAT_OWNER_REMAP)
                        .PrintEndLine(" remap)");
                    p.OpenScope();
                    {
                        p.PrintLine("RemapObservedStatsInternal(in remap);");
                    }
                    p.CloseScope();
                    p.PrintEndLine();

                    p.PrintBeginLine("partial void RemapObservedStatsInternal(in ").Print(STAT_OWNER_REMAP)
                        .PrintEndLine(" remap);");
                    p.PrintEndLine();
                }
                p.CloseScope();
            ...
            p.Print("#region    STAT MODIFIER - STACK").PrintEndLine();
            p.Print("#endregion =====================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("StatModifier.Stack");
            p.OpenScope();
            {
                p.PrintLine("partial struct StatModifier // Stack");
                p.OpenScope();
                {
                    p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
        ...
        private static void WriteWrappers(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("ModifierTriggerEvent");
            p.OpenScope();
            {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("StatModifierRecord");
            p.OpenScope();
            {
            ...
            p.Print("#region    API").PrintEndLine();
            p.Print("#endregion ===").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("API");
            p.OpenScope();
            {
            ...
            p.Print("#region    ACCESSOR READ-ONLY").PrintEndLine();
            p.Print("#endregion ==================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Accessor.ReadOnly");
            p.OpenScope();
            {
                p.PrintLine("partial struct Accessor // ReadOnly");
                p.OpenScope();
                {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Builder");
            p.OpenScope();
            {
                WriteAPI(ref p, "Builder", STAT_BUILDER, "builder", WriteBakerConstructor);
            }
            p.CloseScope();
            p.PrintEndLine();

            p.Print("#region    WORLD DATA").PrintEndLine();
            p.Print("#endregion ==========").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("WorldData");
            p.OpenScope();
            {
            ...
            {
                p.PrintLine(AGGRESSIVE_INLINING);
                p.PrintBeginLine("public Accessor(").Print(STAT_STORE)
                    .PrintEndLine(" store) : this(store, default) { }");
                p.PrintEndLine();

                p.PrintLine(AGGRESSIVE_INLINING);
                p.PrintBeginLine("public Accessor(").Print(STAT_STORE)
                    .PrintEndLine(" store, ValuePair.Composer valuePairComposer)");
                p.OpenScope();
                {
                    p.PrintLine("this.accessor = new(store, valuePairComposer);");
                }
                p.CloseScope();
                p.PrintEndLine();
            }
            ...
            {
                p.PrintLine(AGGRESSIVE_INLINING);
                p.PrintBeginLine("public ReadOnly(").Print(STAT_STORE).PrintEndLine(" store)");
                p.OpenScope();
                {
                    p.PrintLine("this.accessor = new(store);");
                }
                p.CloseScope();
                p.PrintEndLine();
            }
        ...
        private static void WriteJobs(ref Printer p, References references, string syntaxKeyword, string statSystemName)
        {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("DeferredUpdateStatListJob");
            p.OpenScope();
            {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("DeferredUpdateStatQueueJob");
            p.OpenScope();
            {
            ...
            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("DeferredUpdateStatStreamJob");
            p.OpenScope();
            {
            ...
            // The Latios UnsafeParallelBlockList job is not ported (D-104): the runtime has no
            // DeferredUpdateStatUnsafeBlockListJob to wrap. Users who need it write their own IJob
            // calling accessor.TryUpdateStat.
        ...
        private static void WriteValuePair(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            ...
            p.Print("#region    VALUE PAIR COMPOSER").PrintEndLine();
            p.Print("#endregion ===================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("ValuePair.Composer");
            p.OpenScope();
            {
                p.PrintLine("partial struct ValuePair // Composer");
                p.OpenScope();
                {
                    p.PrintLine(SERIALIZABLE);
        ...
        private static void WriteAPIImpl(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            p.Print("#region    IMPL - API").PrintEndLine();
            p.Print("#endregion ==========").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: API");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
                ...
                p.OpenScope();
                {
                    // Replaces GetStatComponentTypeSet / AddStatComponents x3 / BakeStatComponents.
                    // Those took EntityManager, EntityCommandBuffer or IBaker; the store owns
                    // creation now, and this is the only entry point that seeds the None stat (D-102).
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".CreateStatOwnerHandle{").Print(TYPES_6_T).Print("}(ref ")
                        .Print(STAT_STORE_XML).Print(", out ").Print(OWNER).Print(", out ")
                        .Print(STAT_BUILDER_XML).Print(", int, int, int, TValuePairComposer")
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static Builder CreateStatOwner(ref ")
                        .Print(STAT_STORE).Print(" store, out ").Print(OWNER)
                        .PrintEndLine(" owner, int statCapacity = 4, int modifierCapacity = 4"
                            + ", int observerCapacity = 4, ValuePair.Composer valuePairComposer = default)");
                    p.OpenScope();
                    {
                        p.PrintBeginLine(STAT_API).Print(".CreateStatOwnerHandle<")
                            .Print("ValuePair, Stat, StatModifier, StatModifier.Stack, StatObserver, ValuePair.Composer")
                            .PrintEndLine(">(ref store, out owner, out var statBuilder"
                                + ", statCapacity, modifierCapacity, observerCapacity, valuePairComposer);");
                        p.PrintLine("return statBuilder;");
                    }
                    p.CloseScope();
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        ...
                        .Print(OWNER).Print(", TStatData, bool, uint, ref ")
                    ...
                    p.PrintBeginLine("public static ").Print(STAT_HANDLE_T).Print(" CreateStatHandle<TStatData>(")
                        .Print(OWNER).Print(" owner, TStatData statData, bool produceChangeEvents, uint userData, ref ")
                    ...
                    p.WithIncreasedIndent()
                        ...
                        .Print("(owner, statData, produceChangeEvents, userData, statBuffer")
                        .PrintEndLine(", modifierRangeStart, observerRangeStart, valuePairComposer);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".CreateStatHandle{").Print(TYPES_4_T_COMPOSER).Print("}(")
                        .Print(OWNER).Print(", TValuePair, bool, uint, ref ")
                        .Print(STAT_BUFFER_XML).Print(", int, int, out TStatData, TValuePairComposer")
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static ").Print(STAT_HANDLE_T).Print(" CreateStatHandle<TStatData>(")
                        .Print(OWNER).Print(" owner, ValuePair valuePair, bool produceChangeEvents, uint userData, ref ")
                        .Print(STAT_BUFFER).Print(" statBuffer, int modifierRangeStart, int observerRangeStart")
                    ...
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .Print(".CreateStatHandle<ValuePair, Stat, TStatData, ValuePair.Composer>")
                        .Print("(owner, valuePair, produceChangeEvents, userData, statBuffer")
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        ...
                        .Print(OWNER).Print(", TValuePair, bool, uint, ").Print(STAT_BUFFER_XML)
                    ...
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .Print(".CreateStatHandle<ValuePair, Stat>(owner, valuePair, produceChangeEvents, userData")
                        .PrintEndLine(", statBuffer, modifierRangeStart, observerRangeStart);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".OwnerHasAnyOtherDependantStats{TStatObserver}(")
                        .Print(OWNER).Print(", ").Print(LOOKUP_OBSERVER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void OwnerHasAnyOtherDependantStats(")
                        .Print(OWNER).Print(" owner, ")
                        .Print(LOOKUP_OBSERVER).PrintEndLine(" lookupObservers)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".OwnerHasAnyOtherDependantStats<StatObserver>(owner, lookupObservers);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".OwnerHasAnyOtherDependantStats{TStatObserver}(")
                        .Print(OWNER).Print(", ").Print(RO_SPAN_OBSERVER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void OwnerHasAnyOtherDependantStats(")
                        .Print(OWNER).Print(" owner, ")
                        .Print(RO_SPAN_OBSERVER).PrintEndLine(" observerBufferOnOwner)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".OwnerHasAnyOtherDependantStats<StatObserver>(owner, observerBufferOnOwner);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".GetOtherDependantOwnersOfOwner{TStatObserver}(")
                        .Print(OWNER).Print(", ").Print(LOOKUP_OBSERVER_XML).Print(", ").Print(NATIVE_SET_OWNER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void GetOtherDependantOwnersOfOwner(")
                        .Print(OWNER).Print(" owner, ")
                        .Print(LOOKUP_OBSERVER).Print(" lookupObservers, ")
                        .Print(NATIVE_SET_OWNER).PrintEndLine(" dependentOwners)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".GetOtherDependantOwnersOfOwner<StatObserver>(owner, lookupObservers, dependentOwners);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".GetOtherDependantOwnersOfOwner{TStatObserver}(")
                        .Print(OWNER).Print(", ").Print(RO_SPAN_OBSERVER_XML).Print(", ").Print(NATIVE_SET_OWNER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void GetOtherDependantOwnersOfOwner(")
                        .Print(OWNER).Print(" owner, ")
                        .Print(RO_SPAN_OBSERVER).Print(" observerBufferOnOwner, ")
                        .Print(NATIVE_SET_OWNER).PrintEndLine(" dependentOwners)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".GetOtherDependantOwnersOfOwner<StatObserver>(owner, observerBufferOnOwner, dependentOwners);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".GetOtherDependantStatsOfOwner{TStatObserver}(")
                        .Print(OWNER).Print(", ").Print(LOOKUP_OBSERVER_XML).Print(", ").Print(NATIVE_LIST_STAT_HANDLE_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void GetOtherDependantStatsOfOwner(")
                        .Print(OWNER).Print(" owner, ")
                        .Print(LOOKUP_OBSERVER).Print(" lookupObservers, ")
                        .Print(NATIVE_LIST_STAT_HANDLE).PrintEndLine(" dependentStats)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".GetOtherDependantStatsOfOwner<StatObserver>(owner, lookupObservers, dependentStats);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".GetOtherDependantStatsOfOwner{TStatObserver}(")
                        .Print(OWNER).Print(", ").Print(RO_SPAN_OBSERVER_XML).Print(", ").Print(NATIVE_LIST_STAT_HANDLE_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void GetOtherDependantStatsOfOwner(")
                        .Print(OWNER).Print(" owner, ")
                    ...
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".GetOtherDependantStatsOfOwner<StatObserver>(owner, observerBufferOnOwner, dependentStats);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".GetOwnersThatOwnerDependsOn{").Print(TYPES_4_T).Print("}(")
                        .Print(OWNER).Print(", ").Print(LOOKUP_MODIFIER_XML).Print(", ")
                        .Print(NATIVE_SET_OWNER_XML).Print(", ").Print(NATIVE_LIST_STAT_HANDLE_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void GetOwnersThatOwnerDependsOn(")
                        .Print(OWNER).Print(" owner, ")
                        .Print(LOOKUP_MODIFIER).Print(" lookupModifiers, ")
                        .Print(NATIVE_SET_OWNER).Print(" dependsOnOwners, ")
                        .Print(NATIVE_LIST_STAT_HANDLE).PrintEndLine(" observerStatHandles)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .Print(".GetOwnersThatOwnerDependsOn<ValuePair, Stat, StatModifier, StatModifier.Stack>")
                        .PrintEndLine("(owner, lookupModifiers, dependsOnOwners, observerStatHandles);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".GetOwnersThatOwnerDependsOn{").Print(TYPES_4_T).Print("}(")
                        .Print(RO_SPAN_MODIFIER_XML).Print(", ").Print(NATIVE_SET_OWNER_XML).Print(", ")
                        .Print(NATIVE_LIST_STAT_HANDLE_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static void GetOwnersThatOwnerDependsOn(")
                        .Print(RO_SPAN_MODIFIER).Print(" modifierBufferOnOwner, ")
                        .Print(NATIVE_SET_OWNER).Print(" dependsOnOwners, ")
                        .Print(NATIVE_LIST_STAT_HANDLE).PrintEndLine(" observerStatHandles)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .Print(".GetOwnersThatOwnerDependsOn<ValuePair, Stat, StatModifier, StatModifier.Stack>")
                        .PrintEndLine("(modifierBufferOnOwner, dependsOnOwners, observerStatHandles);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        .Print(".TryGetAllObservers{TStatObserver}(")
                        .Print(OWNER).Print(", ").Print(LOOKUP_OBSERVER_XML).Print(", ").Print(NATIVE_LIST_OBSERVER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    ...
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".TryGetAllObservers<StatObserver>(owner, lookupObservers, observers);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_API)
                        ...
                        .Print(OWNER).Print(", ").Print(LOOKUP_STAT_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public static ").Print(NATIVE_ARRAY_STAT_RO).Print(" GetStats(")
                        .Print(OWNER).Print(" owner, ").Print(LOOKUP_STAT).PrintEndLine(" lookupStats)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> ").Print(STAT_API)
                        .PrintEndLine(".GetStats<ValuePair, Stat>(owner, lookupStats);");
                    p.PrintEndLine();
        ...
        private static void WriteReaderImpl(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            p.Print("#region    IMPL - READER").PrintEndLine();
            p.Print("#endregion =============").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: Reader");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
        ...
        private static void WriteAccessorImpl(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            p.Print("#region    IMPL - ACCESSOR").PrintEndLine();
            p.Print("#endregion ===============").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: Accessor");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
                ...
                p.OpenScope();
                {
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        ...
                        .Print(OWNER).Print(", TValuePair, bool, uint, out ").Print(STAT_HANDLE_T_XML).Print(", out TStatData")
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public bool TryCreateStatHandle<TStatData>(")
                        .Print(OWNER).Print(" owner, ValuePair valuePair, ")
                        .Print("bool produceChangeEvents, uint userData, out ").Print(STAT_HANDLE_T)
                        .PrintEndLine(" statHandle, out TStatData statData)");
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> this.accessor.TryCreateStatHandle<TStatData>(owner, valuePair, produceChangeEvents, ")
                        .PrintEndLine("userData, out statHandle, out statData);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TryCreateStatHandle{TStatData}(")
                        .Print(OWNER).Print(", TValuePair, bool, uint, out ").Print(STAT_HANDLE_T_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public bool TryCreateStatHandle<TStatData>(")
                        .Print(OWNER).Print(" owner, ValuePair valuePair, ")
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TryCreateStatHandle{TStatData}(")
                        .Print(OWNER).Print(", TStatData, bool, uint, out ").Print(STAT_HANDLE_T_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public bool TryCreateStatHandle<TStatData>(")
                        .Print(OWNER).Print(" owner, TStatData statData, bool produceChangeEvents, uint userData, out ")
                        .Print(STAT_HANDLE_T).PrintEndLine(" statHandle)");
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> this.accessor.TryCreateStatHandle<TStatData>(owner, statData, produceChangeEvents, ")
                        .PrintEndLine("userData, out statHandle);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TryCreateStatHandle(")
                        .Print(OWNER).Print(", TValuePair, bool, uint, out ").Print(STAT_HANDLE)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public bool TryCreateStatHandle(")
                        .Print(OWNER).Print(" owner, ValuePair valuePair, bool produceChangeEvents, uint userData, out ")
                        .Print(STAT_HANDLE).PrintEndLine(" statHandle)");
                    p.WithIncreasedIndent()
                        .PrintBeginLine("=> this.accessor.TryCreateStatHandle(owner, valuePair, produceChangeEvents, ")
                        .PrintEndLine("userData, out statHandle);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TryGetAllObservers(")
                        .Print(OWNER).Print(", ").Print(NATIVE_LIST_OBSERVER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public readonly bool TryGetAllObservers(")
                        .Print(OWNER).Print(" owner, ").Print(NATIVE_LIST_OBSERVER).PrintEndLine(" observers)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TryGetAllObservers(owner, observers);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("public readonly ").Print(NATIVE_ARRAY_STAT_RO).Print(" GetStats(")
                        .Print(OWNER).PrintEndLine(" owner)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.GetStats(owner);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        ...
                        .Print("S.ReadOnlySpan{").Print(STAT_VALUE_PARAMS_XML)
                        .Print("}, S.Span{bool}, ref ").Print(STAT_WORLD_DATA_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public void TrySetBaseValueToStats(")
                        .Print("S.ReadOnlySpan<").Print(STAT_VALUE_PARAMS)
                        .PrintEndLine("> paramsForStats, S.Span<bool> results, ref WorldData worldData)");
                    p.WithIncreasedIndent()
                    ...
                    p.PrintBeginLine("public bool TrySetStatBaseValue(")
                        .Print(STAT_HANDLE).Print(" statHandle, in ValuePair value, ").Print(STAT_BUFFER)
                        .PrintEndLine(" statBuffer, ref WorldData worldData)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TrySetStatBaseValue(statHandle, value, statBuffer, ref worldData.worldData);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("public bool TrySetStatCurrentValue(")
                        ...
                        .PrintEndLine(" statBuffer, ref WorldData worldData)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TrySetStatCurrentValue(statHandle, value, statBuffer, ref worldData.worldData);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("public bool TrySetStatData<TStatData>(")
                        .Print(STAT_HANDLE_T).Print(" statHandle, TStatData statData, ").Print(STAT_BUFFER)
                        .PrintEndLine(" statBuffer, ref WorldData worldData)");
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TrySetStatData<TStatData>(statHandle, statData, statBuffer, ref worldData.worldData);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        ...
                        .Print(", ").Print(STAT_BUFFER_XML).Print(", ref ").Print(STAT_WORLD_DATA_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    ...
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TrySetStatValues(statHandle, value, statBuffer, ref worldData.worldData);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("public bool TrySetStatValues(")
                        ...
                        .Print(STAT_VARIANT).Print(" currentValue, ").Print(STAT_BUFFER)
                        .PrintEndLine(" statBuffer, ref WorldData worldData)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TrySetStatValues(statHandle, baseValue, currentValue, statBuffer, ref worldData.worldData);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TrySetStatProduceChangeEvents(")
                        .Print(STAT_HANDLE).Print(", bool, ").Print(STAT_BUFFER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public bool TrySetStatProduceChangeEvents(")
                    ...
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TrySetStatProduceChangeEvents(statHandle, value, statBuffer);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TrySetStatUserData(")
                        .Print(STAT_HANDLE).Print(", uint, ").Print(STAT_BUFFER_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public bool TrySetStatUserData(")
                        ...
                        .Print(STAT_BUFFER).PrintEndLine(" statBuffer)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.accessor.TrySetStatUserData(statHandle, value, statBuffer);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TryUpdateAllStats(")
                        .Print(OWNER).Print(", ref ").Print(STAT_WORLD_DATA_XML)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public void TryUpdateAllStats(")
                        .Print(OWNER).PrintEndLine(" owner, ref WorldData worldData)");
                    p.WithIncreasedIndent().PrintLine("=> this.accessor.TryUpdateAllStats(owner, ref worldData.worldData);");
                    p.PrintEndLine();

                    // Without these two the remap pass of DEC-004 is unreachable from the generated
                    // API, and a loaded save silently keeps cross-owner modifiers pointing at
                    // whoever now occupies the old slot.
                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TryRemapOwner(")
                        .Print(OWNER).Print(", in ").Print(STAT_OWNER_REMAP)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public readonly bool TryRemapOwner(")
                        .Print(OWNER).Print(" owner, in ").Print(STAT_OWNER_REMAP)
                        .PrintEndLine(" remap)");
                    p.WithIncreasedIndent().PrintLine("=> this.accessor.TryRemapOwner(owner, in remap);");
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_ACCESSOR_XML)
                        .Print(".TryRemapOwners(S.ReadOnlySpan{")
                        .Print(OWNER).Print("}, in ").Print(STAT_OWNER_REMAP)
                        .PrintEndLine(")\"/>");
                    p.PrintLine(AGGRESSIVE_INLINING);
                    p.PrintBeginLine("public readonly bool TryRemapOwners(S.ReadOnlySpan<")
                        .Print(OWNER).Print("> owners, in ").Print(STAT_OWNER_REMAP)
                        .PrintEndLine(" remap)");
                    p.WithIncreasedIndent().PrintLine("=> this.accessor.TryRemapOwners(owners, in remap);");
                    p.PrintEndLine();
                    ...
                    // Update(ref SystemState) is gone: there is no system scheduling to refresh
                    // lookups against, and the accessor holds one store instead of four lookups (D-101).
                }
                p.CloseScope();
                p.PrintEndLine();
            }
        ...
        private static void WriteAccessorReadOnlyImpl(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            p.Print("#region    IMPL - ACCESSOR READ-ONLY").PrintEndLine();
            p.Print("#endregion =========================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: Accessor.ReadOnly");
            p.OpenScope();
            {
                p.PrintLine("partial struct Accessor // ReadOnly");
                p.OpenScope();
                ...
                p.OpenScope();
                {
                    ...
                    // Update(ref SystemState) is gone — see the note in WriteAccessorImpl (D-101).
                }
                p.CloseScope();
                p.CloseScope();
                p.PrintEndLine();
        ...
        private static void WriteBakerImpl(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            p.Print("#region    IMPL - BAKER").PrintEndLine();
            p.Print("#endregion ============").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: Baker");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
                p.PrintLine("partial struct Builder");
                p.OpenScope();
                {
                    // IBaker / Entity are replaced by the owner handle the builder was created for.
                    p.PrintBeginLine("public readonly ").Print(OWNER).PrintEndLine(" Owner");
                    p.OpenScope();
                    {
                        p.PrintLine(AGGRESSIVE_INLINING);
                        p.PrintLine("get => this.builder.Owner;");
                    }
                    p.CloseScope();
                    p.PrintEndLine();

                    p.PrintBeginLine("/// <inheritdoc cref=\"").Print(STAT_BUILDER_XML)
                        .Print(".Clear(")
                    ...
                    p.PrintBeginLine("public void Clear(")
                        .PrintEndLine("ValuePair.Composer valuePairComposer = default)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.Clear(valuePairComposer);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.CreateStatHandle<TStatData>(statData, produceChangeEvents, userData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.CreateStatHandle<TStatData>(valuePair, produceChangeEvents, userData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.CreateStatHandle<TStatData>(valuePair, produceChangeEvents, userData, out statData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.CreateStatHandle(valuePair, produceChangeEvents, userData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.Contains(statHandle);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.Contains(statHandle, userData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.SetStatOrCreateHandle(handle, statData, produceChangeEvents, userData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.SetStatOrCreateHandle(handle, valuePair, produceChangeEvents, userData, out statData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.SetStatOrCreateHandle(handle, valuePair, produceChangeEvents, userData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent().PrintBeginLine("where TStatData : unmanaged, ").PrintEndLine(ISTAT_DATA);
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.SetStat(handle, statData, produceChangeEvents, userData);");
                    p.PrintEndLine();
                    ...
                    p.PrintBeginLine("public void SetStat(")
                        .Print(STAT_HANDLE).PrintEndLine(" handle, ValuePair valuePair, bool produceChangeEvents, uint userData)");
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.SetStat(handle, valuePair, produceChangeEvents, userData);");
                    p.PrintEndLine();
                    ...
                    p.WithIncreasedIndent()
                        .PrintLine("=> this.builder.TryAddStatModifier(affectedStatHandle, modifier, out statModifierHandle);");
                    p.PrintEndLine();
                }
                p.CloseScope();
                p.PrintEndLine();
        ...
        private static void WriteWorldDataImpl(ref Printer p, string syntaxKeyword, string statSystemName)
        {
            p.Print("#region    IMPL - WORLD DATA").PrintEndLine();
            p.Print("#endregion =================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: WorldData");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
        ...
        {
            p.Print("#region    IMPL - VALUE PAIR").PrintEndLine();
            p.Print("#endregion =================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: ValuePair");
            p.OpenScope();
            {
                p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
        ...
        private static void WriteStatDataStore(
              ref Printer p
            , List<TypeRecord> singleTypes
            , List<TypeRecord> pairTypes
            , string syntaxKeyword
        ...
        {
            p.Print("#region    IMPL - STAT DATA STORE").PrintEndLine();
            p.Print("#endregion ======================").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: StatDataStore");
            p.OpenScope();
        ...
        private static void WritePairTypes(
              ref Printer p
            , List<TypeRecord> pairTypes
            , string syntaxKeyword
            , string statSystemName
        )
        {
            foreach (var (ns, type, typeName, size) in pairTypes)
            {
                ...
                p.Print("#endregion ============").PrintRepeat('=', typeNameUpper.Length).PrintEndLine();
                p.PrintEndLine();

                p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                    .Print(" // ").Print("Impl: Pair").PrintEndLine(typeName);
                p.OpenScope();
                {
                    p.PrintLine(STRUCT_LAYOUT_EXPLICIT);
                    p.PrintBeginLine(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
        ...
        {
            p.Print("#region    IS COMPATIBLE").PrintEndLine();
            p.Print("#endregion =============").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("IsCompatible");
            p.OpenScope();
            {
                p.PrintBeginLine(AGGRESSIVE_INLINING).Print(GENERATED_CODE).PrintEndLine(EXCLUDE_COVERAGE);
        ...
        private static void WriteHelpersMethods(ref Printer p,
              string containerType
            , string syntaxKeyword
            , string statSystemName
        )
        {
            p.Print("#region    IMPL - HELPERS").PrintEndLine();
            p.Print("#endregion ==============").PrintEndLine();
            p.PrintEndLine();

            p.PrintBeginLine("partial ").Print(syntaxKeyword).Print(" ").Print(statSystemName)
                .Print(" // ").PrintEndLine("Impl: Helpers");
            p.OpenScope();
            {
